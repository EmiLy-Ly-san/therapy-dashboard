/**
 * Hook data : bibliothèque des contenus partagés d'un patient (côté therapist)
 *
 * Objectif :
 * - Charger les items visibles par le therapist pour un patient donné
...
export function useTherapistPatientItems(
  filterType: TherapistFilterType,
  ...
  async function loadItems(reason: 'initial' | 'refresh' = 'refresh') {
    ...
    if (!patientId) {
      ...
      setItems([]);
      setThumbUrls({});
      setErrorMessage('Patient manquant.');
      return;
    }

    // ⚠️ Important :
    // - On filtre par patientId pour avoir une “vue patient”
    // - La RLS s’assure que seuls les items partagés sont retournés
